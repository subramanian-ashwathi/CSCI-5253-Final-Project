...
def transform_data(
    spark_master, database_connection_string, app_name="CrimeDataAnalysis", **context
):
    # Initialize Spark session
    spark = SparkSession.builder.appName(app_name).master(spark_master).getOrCreate()

    start_date = (
        datetime.strptime(context["ds"], "%Y-%m-%d") - relativedelta(months=1)
    ).strftime("%Y-%m-%d")
    end_date = context["ds"]
    ...
    print(
        "The shape before removing duplicates from the dataset is:",
    ...
    print(
        "The shape after removing duplicates from the dataset is:",
        (num_rows_after, num_columns),
    )
    ...
    num_rows = data.count()
    null_percentages = [
        (count(when(col(c).isNull(), c)) / num_rows * 100).alias(c)
    ...
    data = data.withColumn(
        "time_occ",
        concat_ws(
            ":", substring(col("time_occ"), 1, 2), substring(col("time_occ"), 3, 2)
        ),
    )
    ...
    for key, value in descent_mapping.items():
        ...
        data = data.withColumn(
            "vict_descent",
            when((col("vict_descent") == key), value).otherwise(col("vict_descent")),
        )
    ...
    data = (
        data.withColumnRenamed("lat", "Latitude")
    ...
    data = data.select(
        [column for column in data.columns if column not in columns_to_drop]
    )
    ...
    data = data.withColumn(
        "Longitude", when(col("Longitude") == 0, None).otherwise(col("Longitude"))
    )
    ...
    data = data.withColumn(
        "Crime Description", remove_text_inside_brackets(col("Crime Description"))
    )
    data = data.withColumn(
        "Crime Description", regexp_replace(col("Crime Description"), ",", "-")
    )
    ...
    data = data.withColumn(
        "Weapon Description", remove_text_inside_brackets(col("Weapon Description"))
    )
    data = data.withColumn(
        "Weapon Description", regexp_replace(col("Weapon Description"), ",", "-")
    )
    data = data.withColumn(
        "Weapon Description", initcap(trim(col("Weapon Description")))
    )
    ...
    data = data.withColumn(
        "Crime Location", regexp_replace(col("Crime Location"), "\\*", "")
    )
    ...
    print(
        "There are {} crimes committed over {} days. On average, there are {} crimes each day.".format(
            data.count(), duration, int(data.count() / duration)
        )
    )

    # Calculate the delay
    data = data.withColumn(
        "Delay", datediff(col("Date Reported"), col("Date Occurred"))
    )
    ...
    data = data.withColumn(
        "Time Slot",
        when(
            (col("Time Occurred").cast("long") >= float("-inf"))
            & (col("Time Occurred").cast("long") < 1.59 * 3600),
            "0-2",
        )
        .when(
            (col("Time Occurred").cast("long") >= 1.59 * 3600)
            & (col("Time Occurred").cast("long") < 3.59 * 3600),
            "2-4",
        )
        .when(
            (col("Time Occurred").cast("long") >= 3.59 * 3600)
            & (col("Time Occurred").cast("long") < 5.59 * 3600),
            "4-6",
        )
        .when(
            (col("Time Occurred").cast("long") >= 5.59 * 3600)
            & (col("Time Occurred").cast("long") < 7.59 * 3600),
            "6-8",
        )
        .when(
            (col("Time Occurred").cast("long") >= 7.59 * 3600)
            & (col("Time Occurred").cast("long") < 9.59 * 3600),
            "8-10",
        )
        .when(
            (col("Time Occurred").cast("long") >= 9.59 * 3600)
            & (col("Time Occurred").cast("long") < 11.59 * 3600),
            "10-12",
        )
        .when(
            (col("Time Occurred").cast("long") >= 11.59 * 3600)
            & (col("Time Occurred").cast("long") < 13.59 * 3600),
            "12-14",
        )
        .when(
            (col("Time Occurred").cast("long") >= 13.59 * 3600)
            & (col("Time Occurred").cast("long") < 15.59 * 3600),
            "14-16",
        )
        .when(
            (col("Time Occurred").cast("long") >= 15.59 * 3600)
            & (col("Time Occurred").cast("long") < 17.59 * 3600),
            "16-18",
        )
        .when(
            (col("Time Occurred").cast("long") >= 17.59 * 3600)
            & (col("Time Occurred").cast("long") < 19.59 * 3600),
            "18-20",
        )
        .when(
            (col("Time Occurred").cast("long") >= 19.59 * 3600)
            & (col("Time Occurred").cast("long") < 21.59 * 3600),
            "20-22",
        )
        .when(
            (col("Time Occurred").cast("long") >= 21.59 * 3600)
    ...
    data.toPandas().to_csv(f"/tmp/data_df_{context['ds']}")
